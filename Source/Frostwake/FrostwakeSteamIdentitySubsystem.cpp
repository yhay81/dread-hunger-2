#include "FrostwakeSteamIdentitySubsystem.h"
...
FFrostwakeSteamIdentityVerificationResult MakeLocalError(const TCHAR* ErrorCode, const TCHAR* Message)
{
    FFrostwakeSteamIdentityVerificationResult Result;
...
bool UFrostwakeSteamIdentitySubsystem::SubmitSessionTicketToBackend(
    ...
    FFrostwakeSteamIdentityVerificationDelegate Callback)
{
    ...
    UE_LOG(LogFrostwakeSecurity, Log, TEXT("%s"), *BuildRedactedRequestLogLine(BackendBaseUrl, Request));
...
bool UFrostwakeSteamIdentitySubsystem::BuildSessionTicketRequestJson(const FFrostwakeSteamIdentitySessionTicketRequest& Request, FString& OutJson)
{
...
bool UFrostwakeSteamIdentitySubsystem::TryParseSessionTicketResponseJson(
    const FString& ResponseJson,
    int32 HttpStatus,
    bool bTransportSucceeded,
    FFrostwakeSteamIdentityVerificationResult& OutResult)
{
    OutResult = FFrostwakeSteamIdentityVerificationResult();
...
FString UFrostwakeSteamIdentitySubsystem::BuildRedactedRequestLogLine(
    const FString& BackendBaseUrl,
    const FFrostwakeSteamIdentitySessionTicketRequest& Request)
{
...
bool UFrostwakeSteamIdentitySubsystem::IsValidPurpose(const FString& Purpose)
{
    return Purpose == TEXT("moderation") || Purpose == TEXT("server_metadata") || Purpose == TEXT("support");
}

bool UFrostwakeSteamIdentitySubsystem::IsEvenLengthHex(const FString& Value)
{
...
FString UFrostwakeSteamIdentitySubsystem::BuildEndpointUrl(const FString& BackendBaseUrl)
{
