...
| ダメージ/死亡/蘇生 | §3.17 + `item_stats.txt`(全武器値) + DT_*フラグ | 大部分→ **typed data駆動damage**(`AdjustDamage`が`DamageMultiplier`適用) + **ReservedHealth/毒ルーティング**(DT_Poison→reserve、Healthは無傷) + **carcass→revive**(DH 50%HP/25%暖、reserve から復元・毒で減少) + **perk抵抗**(`bAffectedByResistance` フックを消費＝ActionEffect が供給する resistance を `AdjustDamage` が乗算)。`dev_smoke_damage_type`/`dev_smoke_down_rescue`/`dev_smoke_perk_resist`(cold 10×50%耐性→Health−5)で実証。残=全武器係数(item_stats)・AOE(Nitro §3.24)・reserve refill |
| アイテム(55) | `item_stats.txt` + `native_enums`(EInventoryType) | 部分→ **コンテナがスタック対応**(review#2a): `FFrostwakeInventoryEntry{ItemId,Count}` が `ItemDefinition.MaxStack` を尊重(`dev_smoke_inventory`: Ration 5個=1スロット・6個目=新スロット)。**手持ちの他者可視化=土台LIVE**(review#2b): server権威の `HeldItemId` が選択スロットから導出され**全員複製(COND_None)**＝DH社会的読み合いの基盤(`dev_smoke_held_item` で導出を実証、選択は client→server RPC)。残=55アイテムseed(`item_stats.txt`)・手持ち表示UI(nameplate＝client consumer) |
...
| ロール+perk(8) | `native_enums`(EPlayerTeamRole/RolePerk) + CROSSCHECK(実数値) | 部分→ **初の perk = ActionEffect が LIVE**(`UFrostwakeColdResistPerkEffect` が damage resistance を §8経路で供給、`ActionComponent` に蓄積→`AdjustDamage` が消費)。残=8ロール定義+初期装備+ロールperk一式・perk DATA型(`PerkDefinition`) seed・stacking(ADDITIVE/MULTIPLICATIVE+pity, §3.20) |
| Thrall呪文 | §3.18 + CROSSCHECK(spells) | 部分→ **ability FW が呪文を表現可能に**(review#1): 基底 `UFrostwakeAction` に cooldown/cost(resource attribute)/duration/instigator を追加し、初の具象 `UFrostwakeFogAbility`(Stamina cost+cooldown+duration)が `AddAction/StartAction` で実走(`dev_smoke_ability` で cost消費・再起動拒否を実証)。残=5呪文の効果+データ(spells.txt)+running状態のクライアント複製 |
...
- ★ **能力FW** — 軽量Action System（確定済 §3.2）。多数の能力/効果が乗ると載せ替え=書き直し。維持。
  - **ルール(2026-05-30 確定・不可逆)**: damage/perk/effect/buff/debuff/ability の**修正値・効果は必ず `UFrostwakeAction`/`UFrostwakeActionEffect` 経由**で実装する（素メソッド禁止）。最初の実例＝`UFrostwakeColdExposureEffect`（§9.5 step5 LIVE）。**基底の形も能力が乗る前に固める**(review#1): `UFrostwakeAction` に cooldown/cost(resource attribute)/duration/instigator を追加し、初の具象 `UFrostwakeFogAbility` で実走確認済（薄い基底のまま5呪文を載せると基底再設計＝書き直しになる罠を回避）。残=running状態の複製。破ると中核FWが死蔵化し後の載せ替えコストが急増（実際 damage を `AdjustDamage` の素メソッドで先行実装して固着しかけた＝レビュー指摘）。新規 dev-smoke を本番 `GameMode` に足し続けるのも別途避け、テスト抽出は P2 で。
...
| 3. DataAsset基底型 | 🟢 LIVE(2026-05-30) | Item/Weapon/Recipe/Role/Perk/DamageType + AssetManager登録。`GetPrimaryAssetId=(Type,Id:FName)`。**editor-bake commandlet `UBakeFrostwakeDataCommandlet`(`-run=BakeFrostwakeData`)が JSON→`.uasset` を生成**(`/Game/Data/Items` 2件・`/Game/Data/DamageTypes` 5件、LFS)、AssetManager がスキャン・解決 |
| 4. データ投入パターン | 🟢 大部分LIVE(消費者2) | JSON源 → **editor-bake `.uasset` → AssetManager 実ロード**: `DataSubsystem` は baked PrimaryDataAsset を**優先**ロード(`Loaded N from AssetManager (baked)` で実証)・JSON は dev fallback ＝「データはアセット」の本命機構が稼働&**cook安全**(.uasset はネイティブに cook)。消費者2(EatRation/AdjustDamage、`dev_smoke_eat`/`dev_smoke_damage_type`)。残=残4型(weapon/recipe/role/perk)のデータ未seed、再bake運用(JSON編集→`-run=BakeFrostwakeData`→commit) |
| 5. Action System core | 🟢 LIVE(2026-05-30, 両半分) | **Effect 半分**: `UFrostwakeColdExposureEffect`(debuff)+`UFrostwakeColdResistPerkEffect`(perk)が実走(`dev_smoke_effect`/`dev_smoke_perk_resist`)。**Action 半分も LIVE(review#1, 2026-05-30)**: 基底 `UFrostwakeAction` に cooldown/cost(resource attribute)/duration/instigator を追加(薄すぎる基底＝§8 one-way door を解消)し、初の具象 `UFrostwakeFogAbility` が `AddAction/StartAction` で実走(Stamina cost 消費・cooldown で再起動拒否、`dev_smoke_ability` で実証)＝Stamina の初の消費者。**以後 combat/perk/effect/ability は Action/ActionEffect 経由＝§8ルール**(素メソッド禁止)。残=running状態のクライアント複製 |
...
| 7. 脱結合spine | 🟢 LIVE(購読者あり 2026-05-30) | `UFrostwakeMatchSubsystem`。発信(GameState/character→`Notify*`) + **初の購読者: `GameMode` が `OnPlayerDied` を購読→即 `EvaluateMatchEnd`**(イベント駆動の勝敗判定、down-rescue smokeの`spine_player_died_handled`で実証)。client側は GameState レプリ+HUD polling(`OnRep_MatchPhase`/`OnRep_ShipSystems` の空 body は**意図的**=コメント明記)。wire/save版管理規約は未着手 |
