...
| 4. データ投入パターン | 🟡 PARTIAL(初の消費者あり) | JSON採用。**EatRation が `DataSubsystem.GetItemDefinition` で初の消費者に**(food=data駆動、eat smoke `dev_smoke_eat result=pass`)。だが `UFrostwakeDataSubsystem` は **6型中 item の1型のみロード**(`ReloadAllDefinitions`→`LoadItemDefinitions`だけ)、実データ **2件/目標55**、他系統(pickup/craft/role…)は未消費。editor-bake .uasset(AssetManager本ロード)・残5型は未 |
