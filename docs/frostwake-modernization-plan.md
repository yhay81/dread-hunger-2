...
| アイテム(55) | `item_stats.txt` + `native_enums`(EInventoryType) | 部分→ **コンテナがスタック対応**(review#2): `FFrostwakeInventoryEntry{ItemId,Count}` が `ItemDefinition.MaxStack` を尊重(`dev_smoke_inventory` で実証: Ration 5個=1スロット・6個目=新スロット)。残=55アイテムseed(`item_stats.txt`)・**手持ちの他者可視化**(held-item を全員複製＝DH社会的読み合い、次スライス) |
