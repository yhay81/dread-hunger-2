...
| ダメージ/死亡/蘇生 | §3.17 + `item_stats.txt`(全武器値) + DT_*フラグ | 部分→ **typed data駆動damage**(`ApplyServerDamage(Damage.*tag)`→`AdjustDamage`が`DamageTypeDefinition.DamageMultiplier`を適用、`dev_smoke_damage_type`で実証)。残=perk抵抗・ReservedHealth毒・carcass蘇生・全武器係数・AOE |
