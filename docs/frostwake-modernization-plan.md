...
> 進め方 = §4の未チェック項目を1つ取り「最小の検証可能スライス → ビルド/smoke緑 → feature branchへ小コミット」。
...
- [ ] (残・非ブロッキング) toolchain build-id文字列の lockstep（`main.rs`/`*.ps1`/`server.rs`）
...
> 2026-05-30 追加。**foundationトラックが本節を起票**（dispatch由来。起票時点で§9は不在だった＝CLAUDE.md
> 「Keep docs coherent / 駆動docは増やさず計画を拡張」に従い計画(SSOT)へ追記）。**lead へ**: 自身のwave編成が
> あれば本節と統合のこと（foundationは§9.1/§9.2＝自トラックの範囲のみ記述、横断のwave割当はleadが追記）。
> 依存順: **土台(§9.1 critical path) → Wave 1 並列**。土台が緑になるまで Wave 1 は乗らない。

### 9.1 クリティカルパス（foundation トラック — Wave 1 の前提）
所有: `Source/Frostwake/Data/*`, `Source/Frostwake/ActionSystem/*`, `Content/Data/*`（新規足場）。各ステップ build_game 緑 + 小コミット。

- [x] **0. モジュール依存**: `GameplayTags`/`EnhancedInput`/`DataRegistry` を `Frostwake.Build.cs` に追加
  （`Json`/`JsonUtilities` は既存）。`EnhancedInput`/`DataRegistry` プラグインを `.uproject` 有効化。GASは入れない。
  **決定: gameplay は当面 main `Frostwake` モジュールに置く**（§3.2「FrostwakeGameplay プラグイン化」は *検討* のまま延期
  — 加法的・最小リスク・Wave 1 を待たせない / プラグイン抽出は後日機械的に可能）。EnhancedInput は依存＋有効化のみ
  （Character の入力 *移行* は Phase 2、shared file につき本トラックでは触れない）。
- [x] **1. DataAsset 基底型**（`UPrimaryDataAsset`, `Source/Frostwake/Data/`）:
  `UFrostwakeItemDefinition` / `UFrostwakeWeaponDefinition(:Item)` / `UFrostwakeRecipeDefinition` /
  `UFrostwakeRoleDefinition` / `UFrostwakePerkDefinition` / `UFrostwakeDamageTypeDefinition`。
  `GetPrimaryAssetId = (Type=種別, Name=Id:FName)`、共有デフォルトは基底／派生で上書き（Weapon は Item の Type を上書き）。
  AssetManager に 6 PrimaryAssetType をスキャン登録（`DefaultGame.ini`、各 `/Game/Data/<種別>`）。
- [x] **2. データ投入パターン**（§9.2）疎通: Items 2 件で「テキスト編集→ゲーム内反映」経路を確立。
- [x] **3. Action System core**（`Source/Frostwake/ActionSystem/`）:
  `UFrostwakeAttributeComponent`（Health/ReservedHealth/Hunger/Warmth/Stamina, replicated + Push-model）/
  `UFrostwakeAction` / `UFrostwakeActionEffect` 基底 + `UFrostwakeActionComponent`（付与/解除/属性改変が通る最小ホスト）。
- [x] **4. 脱結合 spine**: `UFrostwakeMatchSubsystem`（`UWorldSubsystem`、デリゲートで system 自己登録）。
  Wave 1 が GameMode 直接編集を回避。**GameMode から Notify* を呼ぶ1行配線は shared file につき lead へ報告（未実施）**。

> 検証: 各スライス `unreal-gate` の `build_game: Succeeded` で確認（gate全体のexitは generate_project_files 既知バグで赤になり得る）。

### 9.2 データ投入パターン（確定: JSON テキスト → 型付き Definition）
**1方式に確定。** 権威ソース = `Content/Data/<種別>/source/*.json`（プレーンテキスト、手作り .uasset 不要）。
パースは共有コード（`FJsonObjectConverter`）で `UFrostwake*Definition` の UPROPERTY に束縛。

- **foundation スライス（今・runtime sink）:** `UFrostwakeDataSubsystem`（`UGameInstanceSubsystem`）が起動時に
  JSON を読み `NewObject` で Definition を生成・`TMap<FName,...>` に登録。→ **runtime モジュールのみ＝build_game で検証可・
  editor 不要**で「テキスト編集→ゲーム内反映」が today 閉じる。Wave 1 は subsystem に ItemId で問い合わせ（`GetItemDefinition`）。
- **本番 sink（同一パース流用・延期）:** 同じパースを editor commandlet から呼び `Content/Data/<種別>/` に
  オンディスク PrimaryDataAsset を bake → AssetManager 非同期ロード/cook の恩恵。asset-tooling 後続スライス。
- AssetManager の 6 PrimaryAssetType スキャン（§9.1-1）は本番 sink 用に**先行登録**（オンディスク0件でも無害）。
- **cooked 注意:** ルーズ JSON は既定で cook されない → packaging の *Additional Non-Asset Directories* に `Content/Data` を追加、
  または bake sink を使う（`Content/Data/README.md` 参照）。

理由: PrimaryDataAsset 型を canonical に保ちつつ（§3.4）、.uasset 手作りと editor 依存を避け、build_game だけで
検証できる最小ループを today 成立。CSV でなく JSON: レシピの material map 等のネスト/多態に素直（§3.4）。
