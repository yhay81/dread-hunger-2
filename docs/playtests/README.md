...
Before a human run, generate the local ignored run folder:

```bash
python3 Tools/playtest_run_scaffold.py --run-number 1 --target-players 6
```

This writes host/client scripts, notes, and local evidence directories under `Saved/Playtests/P1-024/run-01/`. That path is ignored by git.
