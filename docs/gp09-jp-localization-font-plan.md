...
### HUD (`Source/Frostwake/FrostwakeHudWidget.cpp`)
...
### Menu (`Source/Frostwake/FrostwakeMainMenuWidget.cpp`)
...
- (b) `LOCTEXT`/`NSLOCTEXT` — idiomatic UE, gathered; already in flight for `FrostwakeShipTaskActor.cpp`.
...
Sketch (String Table):
```cpp
namespace FrostwakeUIText { inline FText T(const TCHAR* Key)
  { return FText::FromStringTable(TEXT("/Game/Localization/ST_FrostwakeUI"), Key); } }
// BuildHud(): MakeLine(WidgetTree, FrostwakeUIText::T(TEXT("Hud_RoleCrew")), 18.0f);
```
...
```cpp
FSlateFontInfo FrostwakeUIText::JpFont(float Size) {
...
// MakeLine: TextBlock->SetFont(FrostwakeUIText::JpFont(FontSize));  // fixes every label at once
```
...
State: HUD + menu source strings are now owned in `ST_FrostwakeUI` (English source, via `FrostwakeUIText`,
`LOCTABLE_*` in `FrostwakeUIText.cpp`); ship-task `LOCTEXT` is already gathered. OFL CJK fonts (Noto Sans JP +
...
- **Gather:** `UnrealEditor-Cmd <project> -run=GatherText -config=Config/Localization/<Target>_Gather.ini`
  — GatherTextFromSource picks up the `LOCTEXT` / `LOCTABLE_*` in `Source/Frostwake` (incl. `FrostwakeUIText.cpp`).
