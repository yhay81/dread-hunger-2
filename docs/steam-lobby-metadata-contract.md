...
- `region`
- `serverName`
- `mode` — match mode advertised to joiners: `standard` or `madman` (mirrors `EFrostwakeMatchMode`). Absent ⇒ treated as `standard`. Distinct from `ruleset`: `ruleset` is the lobby category (`standard`/`private_test`/`developer`), `mode` is the in-match game mode.
- `difficulty` — difficulty preset: `easy`, `normal`, or `hard` (mirrors `EFrostwakeDifficulty`). Absent ⇒ treated as `normal`.
