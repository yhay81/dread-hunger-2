...
> Launcher UE cannot build `AbyssLockServer.exe`. Evidence: `docs/cycles/2026-05-26-cycle-78.md`
...
| --- | --- | --- | --- |
| **p95 server tick time** (ms) | BLOCKED — measure first once `AbyssLockServer.exe` exists | ≤ 20 ms p95 over a full 8-player match | Run `AbyssLockServer.exe` with `stat unit` logging enabled; capture `Saved/ProfilingData/` frame-stat CSV; filter to the `GameThread` row; compute p95 across all ticks from match_started to match_ended. Command (once server target unblocked): `AbyssLockServer.exe /Game/Maps/L_IcebreakerWhitebox -log -StatNamedEvents -DumpStatCsvToFile` |
...
| **Peak server memory** (MB) | BLOCKED — requires `AbyssLockServer.exe` | ≤ 1 500 MB RSS at peak 8-player match load | Attach `perfmon` or `Get-Process | Select-Object WorkingSet` to the server process during an 8-player full-match run; record peak WorkingSet. Once server target unblocked, run as part of smoke baseline: `cargo run -p frostwake-tools -- run-local-smoke --profile combined8 --collect-memory`. |
| **Peak client memory** (MB) | TBD — measure first on listen-server host | ≤ 2 000 MB RSS on minimum-spec hardware | Attach `Get-Process UnrealEditor` (Editor target) or `AbyssLock` (Game target) to the listen-server smoke run; record peak WorkingSet across a full match. |
...
- Server-side measurements (tick time, server memory, per-connection bandwidth) depend on
  `AbyssLockServer.exe` existing. That binary cannot be produced from the current Launcher UE
  distribution.
...
3. Once `AbyssLockServer.exe` is available, run the same smoke as a dedicated server and
