...
Runtime 側は `Frostwake` module に集約する。Editor 限定の asset 生成・検証は `FrostwakeEditor` module に分離し、shipping/runtime 依存へ混ぜない。
...
`FrostwakeEditor` module の責務:

- whitebox map generation commandlet
- whitebox map validation commandlet
...
現在の Phase 1 実装では、`WaitingForPlayers` 以外の参加は `match_already_started` として拒否する。これは途中復帰未対応を明示するための一時ポリシーであり、`client_disconnect` と join rejection をログで残す。Phase 2 で dedicated server join path が安定した後、同一 player identity の再参加、観戦復帰、または復帰不可の製品仕様を決める。
