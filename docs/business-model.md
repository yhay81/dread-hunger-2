...
| **プレミアム** | +$10/月（Steamサブスク） | **マルチ試合を作成（ホスト）**／**プレミアム限定コスメ**／**Host Standing** 等の prestige | 公式 **AWS** dedicated | プレミアム作成時のみ発生 |
...
| D9 | 競技ランクマッチは**やらない**（パーティゲーム＝仲間依存でスポーツ性が低い）。代わりに**非競技の prestige/バッジ/称号**（自慢構造）を行う。 | DECIDED |
| D10 | **プレミアム限定コスメ**を perk に追加（加入者限定スキン/月替り等、見た目のみ）。 | DECIDED |
| D11 | **標準試合長 18–25分**。終盤に**気温急落等の terminal escalation で ~30分に強制決着/全滅**（ハードキャップ＋クライマックス＋工作員の時間切れ勝ち）。 | DECIDED |
| D12 | **Open試合供給は "Host Standing"（ホスト評価バッジ）で誘因**。Openを開け良試合を回すほど cosmetic/掲出で得（host-to-win禁止）。 | DECIDED |
...
- **$10/月 プレミアム**（Steamサブスク）。中核entitlement = **マルチ試合の作成権**。特典 = **プレミアム限定コスメ＋Host Standing 等**（§6 Progression & Prestige）。
...
- **標準 18–25分**（製品目標、既存docのまま）。白箱は20–35分。
- 終盤、**気温急落などの terminal escalation** で **~30分に強制的に決着/全滅**するハードキャップを設ける。狙い:
  - 18–25分で乗員が完走（脱出/目的達成）するのが通常。粘着・停滞は ~30分で死。
  - これが工作員の勝利条件「**時間切れまで最終航行を阻止**」（`docs/vision.md`）の"歯"になる。
  - 「1試合に明確な山場が最低1回」（`docs/production-plan.md` Gameplay Gate）を満たすクライマックス。
- 数値18–25/20–35は不変。これは**新メカニクスの追加**であって既存目標の変更ではない。

## 6. Progression & Prestige（非競技・D9/D10/D12）

設計原則: **勝敗ladderにしない**（仲間運で上下させない）。累積・実績・個人技・行動で貯まる **vanity（自慢）** に寄せる。**全MPが公式AWS＝統計が信用できる**（コミュニティ鯖なら捏造可能だった→全公式の副次メリット）。すべて**cosmetic/可視性のみ**でゲーム性能に influence させない。

| トラック | 何で貯まる | 自慢の出方 | 競技性 |
| --- | --- | --- | --- |
| **航海キャリア**（全員） | 完走・救助・通報されない清潔さ等の累積 | 称号＋プロフィールバッジ | なし |
| **ロール熟練**（全員） | Crew/Saboteur 別の個人の働き。**負けても貯まる** | Mastery レベル＋フレア | 低（個人技のみ） |
| **実績バッジ** | マイルストーン（"50航海"、"最後の生存乗員で勝利" 等） | ロビーカード＋プロフィール | なし |
| **プレイ報酬コスメ** | 無料層もマイルストーンで無料コスメ解禁 | 見た目の進行 | なし |

- **Host Standing（D12, Open供給の誘因）**: プレミアムのみホスト可なので、ホスト評価を**プレミアムの flex** にする。
  - 加点: **Open試合**が定足数で開始／ホスト離脱なく完走／ユニーク参加者数／任意の「良ロビー」👍。減点: 通報・過剰キック・即抜け。
  - 段階バッジ（例 Helmsman → Navigator → **Convoy Leader**、3–5段。命名はIP-safe仮）。各段で プロフィール章＋試合カードの「⭐ Trusted Host」＋上位で**クイックキュー/リストの掲出ブースト**＋ホスト限定コスメ。
  - 効果: Openを開けるのが**憧れ（自慢）かつ実利（人が集まる）**になり、**無料勢の入る試合（供給）増＋ホスト品質/モデレーション向上**を同時に解く。
  - 公正性: cosmetic＋掲出のみ＝**host-to-win禁止**。👍はユニーク参加者重み付け・decay・farm上限。**公式サーバ記録で信頼**。
- プレミアム限定コスメ（D10）と無料プレイ報酬コスメは別系統（無料層にも進行の flex を残す）。

---

## 未決（OPEN・要オーナー判断）

- **コスメ投入タイミング/カタログ**（`docs/competitive-analysis.md` の警告: リテンション実証まで cosmetics を急がない＝タイミング注意）。
- **プレミアム価格/地域**の最終値（参考: `docs/steam-store.md` JP 1,480/1,680）。
- **一人モードの実バランス**（D7 の仮説はプレイテストで要検証）。
- **prestige の細部**: バッジ段数・名称（IP-safe）・シーズン周期・👍の重み付け/decay。

> ✅ 解決済み（2026-05-29）: 試合長(D11) / 競技ランク方針(D9) / プレミアム限定コスメ(D10) / Open供給=Host Standing(D12)。
...
| ドキュメント | 箇所 | 旧 | 新（本レコード） |
| --- | --- | --- | --- |
| `docs/vision.md` | §やらないこと | 「スキン課金」「ランクマッチ」を**やらない** | コスメ課金は**やる**（D3）。**競技ランクは確定でやらない**が**非競技prestige/バッジはやる**（D9） |
