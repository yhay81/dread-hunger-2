...
## ⚠️ 設計訂正（オーナーフィードバック 2026-05-29）— 船は「勝手に進まない」

cycle 102/103 で実装した「燃料がある間は時間で自動前進」は **誤り**。以下の Dread Hunger 準拠モデルに作り直す
（このセクションが §3「航行モデル」の自動前進記述を **supersede** する）。

### 正しい航海ループ（炉 + 舵）
- **炉（Furnace）**：燃料アイテムを投入し **着火** する。着火後、その燃料が **燃えている間だけ** 船は動力を持つ。
  燃料は燃え尽きると消える（再投入＋再着火が必要）。
- **舵（Helm）**：炉が燃えている間、プレイヤーが **舵を操作している間だけ** 航路が進む。
  **舵を取らなければ進まない。船は自動では進まない。**
- ソロは「炉に燃料を足す ⇄ 舵を取る」を一人で回す。

### 段階（オーナー承認済み）
- **Stage A（今）**：whitebox 上の抽象 Route。**炉（燃料投入＋着火）＋ 舵（操作中のみ前進、炉が燃えている時）**。自動前進は撤去。
- **Stage B（最終）**：船を **実際に操作（操船）** できるように（物理移動・進路）。
- **Stage C（氷塊ダメージ）**：氷塊に当たると **破損（最大20箇所）**。
  - 破損中は **浸水** し、直さないと **沈没**。
  - 破損箇所が多いほど **浸水が速い**。
  - 破損箇所が多いほど **船速が落ちる**。
  - プレイヤーは破損を **修理**（排水・船体補修）しつつ、燃料・操舵も回す。

### メニュー導線の訂正
- セレクタは Solo/Host の **選択前** ではなく **選択後** に出す：
  - **Solo → 難易度のみ**（狂人モードは出さない）。
  - **Host → モード ＋ 難易度**。
