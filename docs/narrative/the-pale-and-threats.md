...
### 追加スロット（teardown 機能パリティで補完 — [`parity-notes.md`](parity-notes.md)）

DH の裏切り者キットにあって我々に欠けていた3機能を **独自表現** で補完（値は GP-03 調整・§6）。
いずれも血の魔術・呪文詠唱ではなく **冷・暗・静寂・氷** の表現で、DH の呪文（名・演出）はなぞらない。

| 力（the Pale・追加） | 機能（独自版） | 痕跡 | 備考 |
| --- | --- | --- | --- |
| **Smother（沈黙）** | 局所の声/音を蒼が飲み込む（意思疎通封じの独自版） | 不自然な無音域、証言の空白 | 近接VC が核ゆえ強力＝強CD。「蒼は静寂を連れてくる」 |
| **Slip（するり）** | 拘束を抜け、暗がりを短く"通って"移動（機動/脱出の独自版） | 開いた拘束、途切れる足跡 | 戦闘力でなく逃走/接近。低体温の闇に紛れる |
| **Mirage（面）** | 蒼が一瞬"顔"をかぶり、誰かの姿に見える（変装/分身の独自版） | 矛盾する目撃、「二人いた」証言 | **Ph2・IP review 必須**（強い同一性錯誤）。「あれは本当に彼だったか」 |

## 2. 白熊（PvE・Phase 2 の AI 敵オプション）

> **Phase 整合**: `docs/game-design.md` の Phase-1 脅威は **吹雪白out**（低コスト＝Call の基盤）。
> **白熊（AI 敵）は Phase-2 オプション**（同じ「脅威1種」枠を AI 敵側で満たす）。以下は白熊採用時の仕様。
...
→ **統合済み**: 力の数値・ログ enum は [`../sabotage.md`](../sabotage.md)「the Pale の力」節（正典）、loadout は [`../roles.md`](../roles.md)、
最終航行/脅威（白out Phase1・白熊 Phase2）は [`../game-design.md`](../game-design.md) へ反映。本§は物語側の出典。
