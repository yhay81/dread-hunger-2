...
> **teardown パリティで補強**（[`parity-notes.md`](parity-notes.md)）: ① 中央の食堂/たまり場に **賭け事の卓**
> （社交＝近接VC の交差点。嘘と同盟が育つ場）。② 甲板に **鐘（船内警報）**＝緊急召集・異変通知。
> ③ **索具・クロウズネスト（登攀可能な見張り台）**＝船首の Forward Lookout（氷況確認・危険だが情報価値が高い）。
> ④ 罐（Boiler）は **妨害機構を内蔵**＝Quench の主要標的。
