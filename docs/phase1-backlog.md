...
| P1-009 | Add local 8-client launch procedure | `cargo run -p frostwake-tools -- run-local-smoke` launches one listen host plus up to 7 localhost clients; ready-lobby and combined systems smoke pass at 8 players with 2 saboteurs |
...
| ID | Task | Done When |
| --- | --- | --- |
| P1-024 | Run 6-8 player human test 1 | Packet prepared in `docs/playtests/p1-024-human-test-1.md`; `cargo run -p frostwake-tools -- playtest-run-scaffold` creates ignored Windows PowerShell run folders and launch scripts; anonymized summary template prepared in `docs/playtests/p1-024-summary-template.md`; `cargo run -p frostwake-tools -- playtest-summary` can generate the summary skeleton from `log-summary` JSON; `cargo run -p frostwake-tools -- playtest-preflight` checks human-summary gates and redaction before commit; non-human dry-run example exists at `docs/playtests/p1-024-dry-run-summary-example.md`; complete when human log, notes, decision list, and preflight pass are saved |
...
| P1-030 | Validate near-future visual POC direction | `cargo run -p frostwake-tools -- scaffold-frostwake-visual-poc` creates a dry-run/local manifest for a separate visual POC; a duplicate visual POC map then uses approved candidate assets or placeholders to show central corridor, engine/fuel/battery bay, and exterior ice access without changing whitebox automation; screenshots and asset-ledger candidate entries exist |
| P1-031 | Resume on Windows workstation | Windows clone passes `quality-gate --require-ue`, `unreal-gate --platform Win64 --include-server`, dedicated-server boot/client-join/ready-lobby probes, and at least `qa-bot`, `match-timer`, `life-action`, `combined5`, and `ready8` smoke profiles; `docs/windows-validation-template.md` is filled into a new cycle or dated result note; server-target result is recorded |
