...
| P1-031 | Resume on Windows workstation | Windows clone passes `quality_gate.py --require-ue`, `unreal_gate.py --platform Win64 --include-server`, and at least `qa-bot`, `combined5`, and `ready8` smoke profiles; server-target result is recorded |
