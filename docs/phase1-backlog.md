...
| P1-003 | Add `GameState` match id, phase, timer | C++ scaffold builds; match phase, start, end, and role events are covered by smoke logs; timer-driven match end pending |
...
| P1-031 | Resume on Windows workstation | Windows clone passes `quality_gate.py --require-ue`, `unreal_gate.py --platform Win64 --include-server`, and at least `qa-bot`, `combined5`, and `ready8` smoke profiles; `docs/windows-validation-template.md` is filled into a new cycle or dated result note; server-target result is recorded |
