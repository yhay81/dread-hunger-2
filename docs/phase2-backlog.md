...
| ID | Task | Done When |
| --- | --- | --- |
| P2-001 | Confirm Phase 2 entry gates | Windows clone records `Tools/windows/run_phase2_entry_validation.ps1` output, `quality-gate --require-ue`, `unreal-gate --platform Win64 --include-server`, dedicated-server boot, client-join, ready-lobby, and key smoke profile evidence in a cycle file |
| P2-005 | Dedicated server client-join validation | `Tools/windows/run_dedicated_client_join_validation.ps1`, `Tools/windows/run_dedicated_ready_validation.ps1`, or the phase entry wrapper pass on Windows with server JSONL evidence, 8 client connections, role assignment, and ready-lobby match start |
...
| P2-003 | Steam Lobby create/find/join spike | `Tools/windows/run_steam_lobby_validation.ps1` preflight passes; `docs/steam-lobby-subsystem-design.md` is implemented in a Windows-only spike that can create a lobby, list/find it, join from a second client, validate metadata with `cargo run -p frostwake-tools -- lobby-metadata-check`, and travel into the same Unreal authoritative server path without trusting lobby state for gameplay |
| P2-004 | Lobby build/version rejection | Lobby metadata follows `Tools/ops/lobby_metadata.schema.json`; `cargo run -p frostwake-tools -- lobby-join-decision` and `Tools/windows/run_steam_lobby_validation.ps1` prove mismatched build or map id is rejected before travel with a logged reason |

## Auth And Moderation Identity

| ID | Task | Done When |
| --- | --- | --- |
| P2-006 | Steam auth ticket and ownership gate | `docs/steam-auth-ownership-gate.md` is implemented with a backend local mock endpoint contract; real Steam Web API verification still gates trusted public reports, moderation identities, or protected server metadata |
...
| ID | Task | Done When |
| --- | --- | --- |
| P2-014 | Voice Chat Interface provider spike | `docs/voice-provider-validation-template.md` records the Unreal Voice Chat Interface provider choice, setup steps, cost/licensing constraints, mute/block behavior, local config boundary, and Windows test command |
| P2-015 | Proximity voice gameplay acceptance | Completed `docs/voice-provider-validation-template.md` records an 8-player test confirming distance falloff, wall/room attenuation decision, dead/downed/contained voice rules, reconnect behavior, mute/block/report UX, and no external chat-service dependency in the play loop |
