...
Common smoke runs now have named profiles. Use `python3 Tools/ue/run_local_smoke.py --profile qa-bot --skip-build --null-rhi` for the light bot gate, `--profile combined5` for the 5-player non-terminal integration gate, `--profile ready8` for the 8-player ready-lobby scale gate, and `--profile combined8` for the 8-player non-terminal integration gate. `--describe-profile` prints the effective settings without launching Unreal.
