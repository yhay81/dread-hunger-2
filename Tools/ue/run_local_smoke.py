...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--smoke-life-action", action="store_true", help="Run a dev-only interactable life action smoke.")
...
def main() -> int:
    ...
    if args.smoke_life_action:
        host_command.append("-AbyssSmokeLifeAction")
    if args.auto_ready:
    ...
    try:
        ...
        if args.smoke_match_timer:
            ...
            if not match_timer:
                ...
                return 29
        if args.smoke_life_action:
            life_action = wait_for_log(host_log, ("dev_smoke_life_action result=pass",), args.match_timeout)
            if not life_action:
                print(f"[FAIL] life action smoke did not complete within {args.match_timeout}s")
                return 30
