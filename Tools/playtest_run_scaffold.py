...
DEFAULT_BUILD_ID = "AbyssLock-Mac-Development-local"
DEFAULT_PROFILE = "human-p1-024"
DEFAULT_UE_EDITOR = "/Users/Shared/Epic Games/UE_5.7/Engine/Binaries/Mac/UnrealEditor"
...
def run_card(run_id: str, target_players: int, port: int, date: str, build_id: str, map_id: str, profile: str) -> str:
...
Expected: quality gate passes, Editor/Game pass, Server target remains blocked by Launcher UE, and `ready8` passes.
...
```bash
python3 Tools/log_summary.py Saved/Playtests/P1-024/{run_slug_from_id(run_id)}/events.jsonl
python3 Tools/log_summary.py Saved/Playtests/P1-024/{run_slug_from_id(run_id)}/events.jsonl --out Saved/Playtests/P1-024/{run_slug_from_id(run_id)}/summary.json
python3 Tools/playtest_summary.py Saved/Playtests/P1-024/{run_slug_from_id(run_id)}/summary.json --out docs/playtests/p1-024-{run_slug_from_id(run_id)}-summary.md
python3 Tools/playtest_preflight.py Saved/Playtests/P1-024/{run_slug_from_id(run_id)}/summary.json --markdown docs/playtests/p1-024-{run_slug_from_id(run_id)}-summary.md
```
"""


def run_slug_from_id(run_id: str) -> str:
    prefix = f"{PLAYTEST_ID}-"
...
def main() -> int:
    ...
    parser.add_argument("--ue-editor", default=DEFAULT_UE_EDITOR, help="UnrealEditor executable path.")
    ...
    files = {
        run_dir / "README.md": run_card(run_id, target_players, args.port, today, args.build_id, args.map_id, args.profile),
